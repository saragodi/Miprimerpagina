    ...
    <div class="row">
        <div class="col-md-4">
            <div class="pr-5 pt-4 pl-3">
                ...
                <h3>Apariencia</h3>
                <p>Tu tienda puede tener la apariencia que quieras, solo es necesario subir tu carpeta de vista a la carpeta
                    de vistas con un nombre único.</p>
            </div>
        ...
        <div class="col-md-8">
            <div class="card card-body mb-4 theme-methods">
                        ...
                        @foreach ($themes as $theme)
                            <div class="col-md-6">
                                <div class="card">
                                    ...
                                    @if ($theme->image == null)
                                        <img src="{{ asset('assets/img/themes/no-image.jpg') }}" class="card-img-top"
                                            alt="{{ $theme->name }}">
                                    @else
                                        <img src="{{ asset('assets/img/themes/' . $theme->image) }}" class="card-img-top"
                                            alt="{{ $theme->name }}">
                                    @endif
                                    <div class="card-body">
                                        <h6 class="card-title">{{ $theme->name }} <strong>{{ $theme->version }}</strong>
                                        </h6>
                                        <p class="card-text">{{ $theme->description }}</p>
                                        <div class="d-flex justify-content-around">
                                            <a href="{{ route('themes.status', $theme->id) }}"
                                                class=" btn btn-info btn-sm mr-2 px-2" data-toggle="tooltip"
                                                data-original-title="Cambiar Estado"><i class="fas fa-sync"></i></a>
                                            <a href="javascript:void(0)" data-toggle="modal"
                                                data-target="#modalEdit{{ $theme->id }}"
                                                class="btn btn-success btn-sm mr-2 px-2">Edit</a>
                                        </div>
                ...
                <a href="javascript:void(0)" data-toggle="modal" data-target="#modalCreate"
                    class="btn btn-sm btn-outline-light btn-uppercase btn-block mt-3">Configurar una nueva Apariencia</a>
            </div>
        </div>
    </div>
                ...
                <form method="POST" action="{{ route('themes.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="modal-body pd-25">
                        ...
                        <div class="alert alert-warning">
                            <p class="mb-0">Al guardar la información de esta apariencia se activará automáticamente. Si
                                tienes otra apariencia activa en tu plataforma se desactivará.</p>
                        </div>
                    </div>
                ...
                <div class="modal-content bd-0 tx-14">
                    ...
                    <form method="POST" action="{{ route('themes.update', $theme->id) }}"
                        ...
                        <div class="modal-body pd-25">
                            ...
                            <div class="alert alert-warning">
                                <p class="mb-0">Al guardar la información de esta apariencia se activará automáticamente.
                                    Si tienes otra apariencia activa en tu plataforma se desactivará.</p>
                            </div>
                        </div>
                        <div class="modal-footer">
